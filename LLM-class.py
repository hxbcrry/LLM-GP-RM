...
运行前：
1. 将分类数据文件放在脚本同级目录，并在 DATA_PATH 中填写文件名；
2. 安装依赖：pip install pandas numpy scikit-learn openai openpyxl deap；
3. 设置环境变量 DEEPSEEK_API_KEY；
4. 如目标列不是最后一列，请修改 TARGET_COLUMN。
"""
...
GP_PARSIMONY_COEF = 0.0001  # Accuracy 取值为 0-1，复杂度惩罚同步缩小

# 分层 CV Accuracy 至少提高该数值才接受新特征。
...
CAAFE_USER_TEMPLATE = dedent(
    ...
    当前任务：
    根据已有特征，为分类目标 {target} 一次构造三个新的、有物理或统计意义的候选特征。
    ...
    6. 可以使用 +、-、*、np.log1p、np.abs、np.sqrt、np.maximum、np.minimum、np.tanh，也允许使用平方 **2。
    7. 禁止直接使用不受保护的普通除法；但允许使用安全除法形式：
       df['A'] / (np.abs(df['B']) + 1e-6)
       或
       df['A'] / (np.sqrt(np.abs(df['B'])) + 1e-6)。
    8. 禁止使用除 **2 以外的其他幂运算。
    9. 如果需要平方，可以写 df['A'] ** 2，也可以写 df['A'] * df['A']。
    10. 如果使用 np.sqrt，必须写成 np.sqrt(np.abs(...))，避免负数开方。
    11. 如果使用 np.log1p，必须写成 np.log1p(np.abs(...))，避免非法输入。
    12. 不要 import。
    13. 不要定义函数。
    14. 不要读取文件。
    15. 不要输出代码块以外的解释文字。
    16. 每个新特征必须各用一行代码完成，不要使用临时变量。
    17. 代码要尽量稳健，避免 NaN 和 inf。
    18. 返回一个 Python 代码块。
    19. 每个候选特征前都要用注释写明 Feature 和 Usefulness。
    20. 所有 DataFrame 列名必须使用字符串形式并加引号。
    21. 即使列名看起来是数字，也必须写成 df['1']、df['2']，禁止写成 df[1]、df[2]。
    22. 生成代码前必须逐字核对每个列名，列名只能来自“当前可用特征”列表。
...
def summarize_features_for_prompt(
    ...
    for col in used_features:
        ...
        lines.append(
            "df[{}]: dtype={}, missing={:.2f}%, samples={}".format(
                repr(col),
                dtype,
                missing_rate,
                samples,
...
def build_short_term_reflection_prompt(
    *,
    base_accuracy: float,    # 基线准确率
    elite_expr: str,         # 精英个体的GP表达式
    ...
    return dedent(
        """
        当前任务是表格分类的自动特征工程。候选表达式会被加入原始特征集合，
        再由下游模型进行交叉验证；CV Accuracy 越高越好。

        原始特征基线 CV Accuracy：{base_accuracy:.6f}

        当前精英个体：
        - GP 前缀表达式：{elite_expr}
        - CV Accuracy：{elite_accuracy:.6f}
        - 含简约惩罚的适应度：{elite_fitness:.6f}
        - 树节点数：{elite_size}

        对比个体：
        - GP 前缀表达式：{comparison_expr}
        - CV Accuracy：{comparison_accuracy:.6f}
...
def build_reflection_candidate_prompt(
    *,
    ...
    return dedent(
        ...
        7. 禁止普通除法符号 /、任意幂、条件语句、赋值和临时变量；但允许 safe_div(a,b) 和 safe_tanh(a)；
        8. 每行只输出一个表达式；
...
def gp_feature_accepted_feedback(
    *,
    ...
    return (
        ...
        "经过 {} 代交叉和变异后，得到更优特征 {}。"
        "{}"
        "它使 CV Accuracy 从 {:.4f} 提高到 {:.4f}，已接受。"
...
# ============================================================
# 5. 下游分类模型：Logistic Regression
# 所有对比方法应保持相同模型、参数和预处理流程。
# ============================================================
...
def cv_score_accuracy(X_df, y, features, seed):
    """
    使用分层 5 折交叉验证计算 Accuracy。

    分类任务必须使用 StratifiedKFold，以尽量保持每一折中的类别比例一致。
    """
    X = X_df[features].replace([np.inf, -np.inf], np.nan).copy()
    y = np.asarray(y)

    if len(X) < 3:
        raise ValueError("样本数量过少，无法进行分类交叉验证。")
...
def normalize_dataframe_column_references(
    code,
    ...
    valid_columns = {
        str(col)
        for col in list(allowed_input_features) + [expected_feature_name]
    }

    pattern = re.compile(r"df\[\s*(-?\d+)\s*\]")

    def replace_match(match):
        numeric_text = match.group(1)
        if numeric_text in valid_columns:
            return "df[{}]".format(repr(numeric_text))
        return match.group(0)

    return pattern.sub(replace_match, code)

def _is_small_positive_constant(node, max_value=1e-3):
    """判断 AST 节点是否为较小正数常量，例如 1e-6。"""
    return (
        isinstance(node, ast.Constant)
        and isinstance(node.value, (int, float))
        and 0 < float(node.value) <= max_value
    )


def _is_safe_abs_like_call(node):
    """允许 np.abs(x)、abs(x) 或 np.sqrt(np.abs(x)) 作为安全除法分母主体。"""
    if not isinstance(node, ast.Call):
        return False

    call_name = get_call_name(node.func)

    if call_name in {"np.abs", "abs"} and len(node.args) == 1:
        return True

    if call_name == "np.sqrt" and len(node.args) == 1:
        inner = node.args[0]
        return _is_safe_abs_like_call(inner)

    return False


def _is_safe_division_denominator(node):
    """
    仅允许形如 np.abs(x) + 1e-6 或 np.sqrt(np.abs(x)) + 1e-6 的安全分母。
    这样既能表达比例关系，又能避免直接除零。
    """
    ...
    return (
        (_is_safe_abs_like_call(left) and _is_small_positive_constant(right))
        or (_is_safe_abs_like_call(right) and _is_small_positive_constant(left))
    )
    ...
    for node in ast.walk(tree):
        ...
        if (
            ...
            if not (
                ...
                raise ValueError(
                    "DataFrame 列名必须使用字符串并加引号。"
                    "例如真实列名为字符串 1 时，应写 df['1']，禁止写 df[1]。"
                )

            column_name = slice_node.value
            if (
                column_name != expected_feature_name
                and column_name not in allowed_input_features
            ):
                raise ValueError(
                    "代码使用了不允许的列：{}".format(column_name)
                )

        if isinstance(node, ast.BinOp) and isinstance(node.op, ast.Div):
            if not _is_safe_division_denominator(node.right):
                raise ValueError(
                    "只允许安全除法：A / (np.abs(B) + 1e-6) "
                    "或 A / (np.sqrt(np.abs(B)) + 1e-6)"
                )
...
def apply_code_safe(X_input, code, allowed_input_features, expected_feature_name):
    ...
    code = normalize_dataframe_column_references(
        code=code,
...
def build_gp_pset(input_features):
    ...
    pset.addPrimitive(gp_safe_div, 2, name="safe_div")
    pset.addPrimitive(gp_maximum, 2, name="maximum")
    pset.addPrimitive(gp_minimum, 2, name="minimum")
    ...
    pset.addPrimitive(gp_tanh, 1, name="safe_tanh")
...
def ast_expr_to_gp_string(node, feature_to_var):
    ...
    if isinstance(node, ast.BinOp):
        ...
        if isinstance(node.op, ast.Div):
            return "safe_div({}, {})".format(left, right)
    ...
    if isinstance(node, ast.Call):
        ...
        # np.maximum / np.minimum 是二元函数，需要在单参数检查前单独处理。
        if call_name == "np.maximum":
            if len(node.args) != 2:
                raise ValueError("np.maximum 必须有两个参数")
            left = ast_expr_to_gp_string(node.args[0], feature_to_var)
            right = ast_expr_to_gp_string(node.args[1], feature_to_var)
            return "maximum({}, {})".format(left, right)

        if call_name == "np.minimum":
            if len(node.args) != 2:
                raise ValueError("np.minimum 必须有两个参数")
            left = ast_expr_to_gp_string(node.args[0], feature_to_var)
            right = ast_expr_to_gp_string(node.args[1], feature_to_var)
            return "minimum({}, {})".format(left, right)

        if len(node.args) != 1:
            raise ValueError("GP 只支持单参数函数调用，np.maximum/np.minimum 除外")
        ...
        if call_name == "np.tanh":
            return "safe_tanh({})".format(arg)
    ...
    def rec(pos):
        ...
        if isinstance(node, gp.Primitive):
            ...
            if node.name == "safe_div":
                return "({} / (np.abs({}) + 1e-6))".format(args[0], args[1]), next_pos
            if node.name == "maximum":
                return "np.maximum({}, {})".format(args[0], args[1]), next_pos
            if node.name == "minimum":
                return "np.minimum({}, {})".format(args[0], args[1]), next_pos
            if node.name == "square":
            ...
            if node.name == "safe_tanh":
                return "np.tanh({})".format(args[0]), next_pos
            if node.name == "neg":
...
def evolve_top2_candidates_with_deap(
    ...
    def evaluate_individual(individual):
        ...
        try:
            ...
            candidate_accuracy = cv_score_accuracy(
                X_df=X_tmp,
                y=y_train,
            ...
            fitness = (1.0 - candidate_accuracy) + GP_PARSIMONY_COEF * len(individual)
            eval_cache[key] = {
                "fitness": fitness,
                "cv_accuracy": candidate_accuracy,
    ...
    for gen_idx in range(1, GP_N_GENERATIONS + 1):
        ...
        if should_reflect:
            ...
            try:
                ...
                short_prompt = build_short_term_reflection_prompt(
                    base_accuracy=base_accuracy,
                    elite_expr=elite_expr,
                    elite_accuracy=elite_info["cv_accuracy"],
                    elite_fitness=elite_info["fitness"],
                    elite_size=len(elite_ind),
                    comparison_expr=comparison_expr,
                    comparison_accuracy=comparison_info["cv_accuracy"],
                ...
                if not reflected_individuals:
                    ...
                    for new_ind in reflected_individuals:
                        ...
                        print(
                            "[反思注入] 表达式={}，CV Accuracy={:.4f}，"
                            "fitness={:.6f}".format(
                                str(new_ind),
                                info.get("cv_accuracy", 0.0),
        ...
        if gen_idx % 10 == 0 or gen_idx == GP_N_GENERATIONS:
            best_now = hof[0]
            best_info = eval_cache.get(str(best_now), {"cv_accuracy": 0.0, "fitness": 999999.0})
            print(
                "DEAP 第 {:02d} 代：best CV Accuracy = {:.4f}, tree_size = {}, "
                "reflection_calls = {}, injected = {}".format(
                    gen_idx,
                    best_info["cv_accuracy"],
                    len(best_now),
                    reflection_calls,
                    reflection_injected
                )
    ...
    best_accuracy = best_info["cv_accuracy"]
    ...
    print("DEAP 最优 CV Accuracy = {:.4f}, 改善量 = {:.4f}".format(best_accuracy, improvement))
    print(
...
def run_caafe_on_train(X_train_base, y_train, seed):
    ...
    for round_idx in range(1, MAX_ROUNDS + 1):
        ...
        base_accuracy = cv_score_accuracy(
            X_df=X_current,
            y=y_train,
        ...
        print("生成前 CV Accuracy = {:.4f}".format(base_accuracy))
        ...
        try:
            ...
            best_accuracy = best_llm_candidate["candidate_accuracy"]
            best_improvement = best_llm_candidate["improvement"]

            print("\n本轮 LLM 原始最佳候选特征：{}".format(best_name))
            print("LLM 最佳候选 CV Accuracy = {:.4f}".format(best_accuracy))
            ...
            final_accuracy = final_candidate["candidate_accuracy"]
            ...
            print("最终最佳 CV Accuracy = {:.4f}".format(final_accuracy))
...
def main():
    ...
    if df[TARGET].isna().any():
        raise ValueError("分类目标列中存在缺失标签，请先删除或补全。")
    ...
    if (
        ...
        and df[TARGET].nunique() > max(20, int(0.2 * len(df)))
    ):
        raise ValueError(
            "目标列包含大量不同数值，看起来仍是连续回归目标。"
            "请将 DATA_PATH 或 TARGET_COLUMN 改为真正的分类数据。"
        )
    ...
    if non_numeric_features:
        raise ValueError(
            "当前 GP 运算仅支持数值型输入特征。请先对以下类别特征进行编码：{}".format(
                non_numeric_features
            )
        )
    ...
    for seed in SEEDS:
        ...
        print(
            "\n测试集结果：Accuracy={:.4f}, Balanced Accuracy={:.4f}, "
            "Precision-Macro={:.4f}, Recall-Macro={:.4f}, F1-Macro={:.4f}"
            .format(
                metrics["accuracy"],
    ...
    print("Accuracy          = {:.4f}".format(results_df["accuracy"].mean()))
    print("Balanced Accuracy = {:.4f}".format(results_df["balanced_accuracy"].mean()))
    print("Precision-Macro   = {:.4f}".format(results_df["precision_macro"].mean()))
    print("Recall-Macro      = {:.4f}".format(results_df["recall_macro"].mean()))
    print("F1-Macro          = {:.4f}".format(results_df["f1_macro"].mean()))

    print("\n" + "=" * 80)
    print("标准差")
    print("=" * 80)

    print("Accuracy std          = {:.4f}".format(results_df["accuracy"].std()))
    print("Balanced Accuracy std = {:.4f}".format(results_df["balanced_accuracy"].std()))
    print("Precision-Macro std   = {:.4f}".format(results_df["precision_macro"].std()))
    print("Recall-Macro std      = {:.4f}".format(results_df["recall_macro"].std()))
    print("F1-Macro std          = {:.4f}".format(results_df["f1_macro"].std()))
